import { client } from "../services/elastic.search.service.js";
export async function insertToElasticSearch(product) {
    try {
        const { id, brand, title, isApproved, isAvailable, type, description, year, ps, pricePerDay, color, photoUrls, rentStart, rentEnd } = product;
        await client.index({
            index: "products",
            id: id.toString(),
            document: {
